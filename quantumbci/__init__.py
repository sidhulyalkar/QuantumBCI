"""QuantumBCI: falsifiable quantum and quantum-inspired neural modelling."""

from .benchmarking import (
    BenchmarkMetrics,
    DensityBenchmarkResult,
    E001RepresentationBenchmarkResult,
    IndexSplit,
    benchmark_density_embeddings,
    benchmark_e001_embeddings,
)
from .bmrb import (
    BMRB_DYNAMICS_ARTIFACT_SCHEMA,
    DEFAULT_E002_RELIABILITY_ESTIMATES,
    BMRBCaseSpec,
    BMRBDynamicsBundle,
    build_bmrb_dynamics_bundle,
    load_bmrb_case_manifest,
    render_bmrb_dynamics_html,
    verify_bmrb_dynamics_mapping,
    write_bmrb_dynamics_bundle,
)
from .bmrb_causal import (
    BMRBCausalBundle,
    BMRBCausalCaseSpec,
    build_bmrb_causal_bundle,
    load_bmrb_causal_manifest,
    render_bmrb_causal_html,
    write_bmrb_causal_bundle,
)
from .bmrb_representation import (
    BMRB_REPRESENTATION_BENCHMARK,
    BMRB_REPRESENTATION_SCHEMA,
    BMRBRepresentationBundle,
    BMRBRepresentationLaneSpec,
    build_bmrb_representation_bundle,
    load_bmrb_representation_manifest,
    render_bmrb_representation_html,
    verify_bmrb_representation_mapping,
    write_bmrb_representation_bundle,
)
from .causal_recapitulation import (
    CAUSAL_EVIDENCE_METHOD_ID,
    CausalCaseEvidence,
    CausalNecessityPolicy,
    CausalNecessityResult,
    MatchedClassicalRecovery,
    ParticipantCausalSummary,
    attach_causal_evidence,
    causal_case_from_neuros_mechint,
    evaluate_causal_necessity,
)
from .claims import ClaimClass, MechanismCard, mechanism_card
from .classical_dynamics import (
    DIRECT_DISCRETE_ESTIMATOR_ID,
    DISCRETE_ROLLOUT_ID,
    ClassicalControlLaneResult,
    ExtendedClassicalControlsResult,
    evaluate_discrete_transition,
    fit_diagonal_ar1,
    fit_full_affine_var1,
    persistence_transition,
    run_extended_classical_controls,
)
from .contextuality import commutator_norm, order_effect, projector
from .dynamics_equivalence import (
    BlochAffineGenerator,
    LindbladAffineEquivalenceAudit,
    LindbladGaugeAudit,
    affine_rhs,
    audit_lindblad_gauge_nonidentifiability,
    audit_qubit_lindblad_affine_equivalence,
    bloch_to_density,
    compile_qubit_lindblad_to_affine,
    density_to_bloch,
    evolve_affine_bloch,
)
from .dynamics_fitting import (
    FIT_ESTIMATOR_ID,
    SCORE_INTEGRATOR_ID,
    DynamicsLaneResult,
    DynamicsMetrics,
    MatchedDynamicsBenchmarkResult,
    evaluate_generator,
    fit_affine_generator,
    fit_canonical_qubit_generator,
    run_matched_qubit_dynamics_benchmark,
)
from .e002_synthetic import (
    CANONICAL_STRUCTURE_RESIDUAL_MAX,
    CLASSICAL_ADVERSARY_RESIDUAL_MIN,
    CanonicalQubitParameters,
    canonical_qubit_model,
    canonical_structure_residual,
    fit_affine_generator_from_trajectories,
    recover_canonical_parameters,
    run_e002_synthetic_recovery_grid,
    simulate_canonical_bloch_trajectories,
)
from .equivalence import (
    BatchEquivalenceAudit,
    DensityCovarianceAudit,
    audit_density_covariance_equivalence,
    audit_embedding_batch,
    trace_normalized_second_moment,
)
from .exporting import (
    export_run_bids_derivative_container,
    export_run_ro_crate,
    verify_run_artifacts,
)
from .kalman import QLSADiagnostics, kalman_filter, qlsa_diagnostics
from .longitudinal import (
    LongitudinalE001CaseResult,
    LongitudinalE001Row,
    PairedBootstrapSummary,
    evaluate_density_information_gate,
    paired_participant_bootstrap,
    run_longitudinal_e001_case,
)
from .matched_recovery import (
    MATCHED_RECOVERY_METHOD_ID,
    MATCHED_RECOVERY_SCHEMA,
    MatchedClassicalRecoveryEvidence,
    build_matched_classical_recovery_evidence,
    matched_classical_recovery_from_mapping,
)
from .neuros_mechint_artifacts import (
    DOSE_RESPONSE_SCHEMA,
    EVIDENCE_PACK_SCHEMA,
    derive_evidence_pack_validation,
    neuros_mechint_stable_hash,
    verify_dose_response_result,
    verify_evidence_pack_result,
)
from .nonlinear_dynamics import (
    FEATURE_COUNTS,
    LENGTH_SCALE_MULTIPLIERS,
    NONLINEAR_MODEL_ID,
    NONLINEAR_SCORE_ID,
    RIDGES,
    RFF_SEED,
    NonlinearCandidateResult,
    NonlinearControlResult,
    NonlinearMetrics,
    NonlinearResidualModel,
    evaluate_nonlinear_model,
    run_nonlinear_residual_control,
)
from .open_system import dephasing_collapse, evolve_lindblad, lindblad_rhs
from .probabilistic_ssm import (
    DIRECT_GAUSSIAN_BASELINE_ID,
    PROBABILISTIC_MODEL_ID,
    PredictiveDensityMetrics,
    ProbabilisticStateSpaceResult,
    fit_base_innovation_variance,
    score_direct_gaussian_var,
    score_identity_observation_kalman,
)
from .recapitulation import (
    EvidenceGate,
    EvidenceTier,
    GateStatus,
    MechanismNecessityProfile,
    RecapitulationSignature,
    bmrb_dynamics_signature,
    bmrb_representation_signature,
    gate_map,
    mechanism_profile_from_mapping,
    validate_monotonic_promotion,
)
from .recipes import FrozenEmbeddingRecipe, RecipeRunResult, load_recipe, run_recipe
from .reliability import (
    DEFAULT_RELIABILITY_BOOTSTRAP_RESAMPLES,
    DEFAULT_RELIABILITY_SEED,
    ICC_METHOD_ID,
    RELIABILITY_BOOTSTRAP_METHOD_ID,
    ICCResult,
    RepeatedCaseEstimate,
    RepeatedCaseReliabilityBundle,
    RepeatedCaseReliabilityResult,
    audit_repeated_case_estimate,
    audit_repeated_case_reliability,
    estimates_from_stability_artifact,
)
from .representation_conservation import (
    REPRESENTATION_CONSERVATION_METHOD_ID,
    RepresentationConservationPolicy,
    RepresentationConservationResult,
    RepresentationEffectCase,
    RepresentationLaneSummary,
    build_representation_necessity_profile,
    evaluate_representation_conservation,
)
from .representation_studies import (
    E001_REPRESENTATION_LANE_SCHEMA,
    encode_frozen_epochs,
    write_e001_representation_lane_bundle,
)
from .spectral import amplitude_encode, classical_fft, qft_probabilities, qft_state
from .stability import (
    BOOTSTRAP_METHOD_ID,
    DEFAULT_BOOTSTRAP_REPLICATES,
    DEFAULT_BOOTSTRAP_SEED,
    DEFAULT_MIN_SUCCESS_FRACTION,
    BootstrapReplicate,
    BootstrapScalarSummary,
    E002BootstrapStabilityResult,
    NonlinearSelectionStability,
    run_e002_bootstrap_stability,
)
from .states import density_from_samples, l1_coherence, purity, von_neumann_entropy
from .switching_dynamics import (
    LABEL_CANONICALIZATION_ID,
    REGIME_COUNT,
    SWITCHING_MODEL_ID,
    SWITCHING_SCORE_ID,
    SwitchingFitResult,
    SwitchingPredictiveMetrics,
    SwitchingStateControlResult,
    fit_switching_affine_var,
    run_switching_state_control,
    score_switching_affine_var,
)
from .trajectory_authority import (
    TrajectoryEvidenceAuthority,
    TrajectoryEvidenceData,
    load_trajectory_contract_descriptor,
)

__all__ = [
    "BOOTSTRAP_METHOD_ID",
    "BMRBCaseSpec",
    "BMRBCausalBundle",
    "BMRBCausalCaseSpec",
    "BMRBDynamicsBundle",
    "BMRBRepresentationBundle",
    "BMRBRepresentationLaneSpec",
    "BMRB_DYNAMICS_ARTIFACT_SCHEMA",
    "BMRB_REPRESENTATION_BENCHMARK",
    "BMRB_REPRESENTATION_SCHEMA",
    "BatchEquivalenceAudit",
    "BenchmarkMetrics",
    "BlochAffineGenerator",
    "BootstrapReplicate",
    "BootstrapScalarSummary",
    "CANONICAL_STRUCTURE_RESIDUAL_MAX",
    "CAUSAL_EVIDENCE_METHOD_ID",
    "CLASSICAL_ADVERSARY_RESIDUAL_MIN",
    "CanonicalQubitParameters",
    "CausalCaseEvidence",
    "CausalNecessityPolicy",
    "CausalNecessityResult",
    "ClaimClass",
    "ClassicalControlLaneResult",
    "DEFAULT_BOOTSTRAP_REPLICATES",
    "DEFAULT_BOOTSTRAP_SEED",
    "DEFAULT_E002_RELIABILITY_ESTIMATES",
    "DEFAULT_MIN_SUCCESS_FRACTION",
    "DEFAULT_RELIABILITY_BOOTSTRAP_RESAMPLES",
    "DEFAULT_RELIABILITY_SEED",
    "DIRECT_DISCRETE_ESTIMATOR_ID",
    "DIRECT_GAUSSIAN_BASELINE_ID",
    "DISCRETE_ROLLOUT_ID",
    "DOSE_RESPONSE_SCHEMA",
    "DensityBenchmarkResult",
    "DensityCovarianceAudit",
    "DynamicsLaneResult",
    "DynamicsMetrics",
    "E001RepresentationBenchmarkResult",
    "E001_REPRESENTATION_LANE_SCHEMA",
    "E002BootstrapStabilityResult",
    "EVIDENCE_PACK_SCHEMA",
    "EvidenceGate",
    "EvidenceTier",
    "ExtendedClassicalControlsResult",
    "FEATURE_COUNTS",
    "FIT_ESTIMATOR_ID",
    "FrozenEmbeddingRecipe",
    "GateStatus",
    "ICC_METHOD_ID",
    "ICCResult",
    "IndexSplit",
    "LABEL_CANONICALIZATION_ID",
    "LENGTH_SCALE_MULTIPLIERS",
    "LindbladAffineEquivalenceAudit",
    "LindbladGaugeAudit",
    "LongitudinalE001CaseResult",
    "LongitudinalE001Row",
    "MATCHED_RECOVERY_METHOD_ID",
    "MATCHED_RECOVERY_SCHEMA",
    "MatchedClassicalRecovery",
    "MatchedClassicalRecoveryEvidence",
    "MatchedDynamicsBenchmarkResult",
    "MechanismCard",
    "MechanismNecessityProfile",
    "NONLINEAR_MODEL_ID",
    "NONLINEAR_SCORE_ID",
    "NonlinearCandidateResult",
    "NonlinearControlResult",
    "NonlinearMetrics",
    "NonlinearResidualModel",
    "NonlinearSelectionStability",
    "PROBABILISTIC_MODEL_ID",
    "PairedBootstrapSummary",
    "ParticipantCausalSummary",
    "PredictiveDensityMetrics",
    "ProbabilisticStateSpaceResult",
    "QLSADiagnostics",
    "REGIME_COUNT",
    "RELIABILITY_BOOTSTRAP_METHOD_ID",
    "REPRESENTATION_CONSERVATION_METHOD_ID",
    "RIDGES",
    "RFF_SEED",
    "RecapitulationSignature",
    "RecipeRunResult",
    "RepeatedCaseEstimate",
    "RepeatedCaseReliabilityBundle",
    "RepeatedCaseReliabilityResult",
    "RepresentationConservationPolicy",
    "RepresentationConservationResult",
    "RepresentationEffectCase",
    "RepresentationLaneSummary",
    "SCORE_INTEGRATOR_ID",
    "SWITCHING_MODEL_ID",
    "SWITCHING_SCORE_ID",
    "SwitchingFitResult",
    "SwitchingPredictiveMetrics",
    "SwitchingStateControlResult",
    "TrajectoryEvidenceAuthority",
    "TrajectoryEvidenceData",
    "affine_rhs",
    "amplitude_encode",
    "attach_causal_evidence",
    "audit_density_covariance_equivalence",
    "audit_embedding_batch",
    "audit_lindblad_gauge_nonidentifiability",
    "audit_qubit_lindblad_affine_equivalence",
    "audit_repeated_case_estimate",
    "audit_repeated_case_reliability",
    "benchmark_density_embeddings",
    "benchmark_e001_embeddings",
    "bloch_to_density",
    "bmrb_dynamics_signature",
    "bmrb_representation_signature",
    "build_bmrb_causal_bundle",
    "build_bmrb_dynamics_bundle",
    "build_bmrb_representation_bundle",
    "build_matched_classical_recovery_evidence",
    "build_representation_necessity_profile",
    "canonical_qubit_model",
    "canonical_structure_residual",
    "causal_case_from_neuros_mechint",
    "classical_fft",
    "commutator_norm",
    "compile_qubit_lindblad_to_affine",
    "dephasing_collapse",
    "density_from_samples",
    "density_to_bloch",
    "derive_evidence_pack_validation",
    "encode_frozen_epochs",
    "estimates_from_stability_artifact",
    "evaluate_causal_necessity",
    "evaluate_density_information_gate",
    "evaluate_discrete_transition",
    "evaluate_generator",
    "evaluate_nonlinear_model",
    "evaluate_representation_conservation",
    "evolve_affine_bloch",
    "evolve_lindblad",
    "export_run_bids_derivative_container",
    "export_run_ro_crate",
    "fit_affine_generator",
    "fit_affine_generator_from_trajectories",
    "fit_base_innovation_variance",
    "fit_canonical_qubit_generator",
    "fit_diagonal_ar1",
    "fit_full_affine_var1",
    "fit_switching_affine_var",
    "gate_map",
    "kalman_filter",
    "l1_coherence",
    "lindblad_rhs",
    "load_bmrb_case_manifest",
    "load_bmrb_causal_manifest",
    "load_bmrb_representation_manifest",
    "load_recipe",
    "load_trajectory_contract_descriptor",
    "matched_classical_recovery_from_mapping",
    "mechanism_card",
    "mechanism_profile_from_mapping",
    "neuros_mechint_stable_hash",
    "order_effect",
    "paired_participant_bootstrap",
    "persistence_transition",
    "projector",
    "purity",
    "qft_probabilities",
    "qft_state",
    "qlsa_diagnostics",
    "recover_canonical_parameters",
    "render_bmrb_causal_html",
    "render_bmrb_dynamics_html",
    "render_bmrb_representation_html",
    "run_e002_bootstrap_stability",
    "run_e002_synthetic_recovery_grid",
    "run_extended_classical_controls",
    "run_longitudinal_e001_case",
    "run_matched_qubit_dynamics_benchmark",
    "run_nonlinear_residual_control",
    "run_recipe",
    "run_switching_state_control",
    "score_direct_gaussian_var",
    "score_identity_observation_kalman",
    "score_switching_affine_var",
    "simulate_canonical_bloch_trajectories",
    "trace_normalized_second_moment",
    "validate_monotonic_promotion",
    "verify_bmrb_dynamics_mapping",
    "verify_bmrb_representation_mapping",
    "verify_dose_response_result",
    "verify_evidence_pack_result",
    "verify_run_artifacts",
    "von_neumann_entropy",
    "write_bmrb_causal_bundle",
    "write_bmrb_dynamics_bundle",
    "write_bmrb_representation_bundle",
    "write_e001_representation_lane_bundle",
]
