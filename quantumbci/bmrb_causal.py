...
def _load_upstream_profile(path: Path) -> tuple[dict[str, Any], MechanismNecessityProfile]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("upstream BMRB artifact must be a JSON object")
...
def load_bmrb_causal_manifest(
    ...
    if not isinstance(payload, Mapping):
        raise ValueError("BMRB causal manifest must be a JSON object")
    ...
    for index, raw in enumerate(raw_cases):
        ...
        recovery = raw.get("matched_recovery")
        if not isinstance(recovery, Mapping):
            raise ValueError(f"cases[{index}].matched_recovery must be an object")
        cases.append(
            BMRBCausalCaseSpec(
                ...
                matched_recovery=MatchedClassicalRecovery.from_mapping(recovery),
            )
        )
...
def build_bmrb_causal_bundle(manifest_path: str | Path) -> BMRBCausalBundle:
    ...
    for case in case_specs:
        dose_payload = json.loads(case.dose_response_path.read_text(encoding="utf-8"))
        faith_payload = json.loads(case.faithfulness_path.read_text(encoding="utf-8"))
        if not isinstance(dose_payload, Mapping) or not isinstance(faith_payload, Mapping):
            raise ValueError("causal evidence artifacts must contain JSON objects")
        causal_cases.append(
            causal_case_from_neuros_mechint(
                ...
                matched_recovery=case.matched_recovery,
            )
        )
    ...
    identity = {
        "schema_version": 1,
        "study_id": study_id,
        "upstream_bmrb_sha256": _sha256_file(upstream_path),
...
<p class="small">Matched-classical recovery is the fraction of candidate ablation loss restored by the strongest declared classical alternative under the same information budget. Lower is stronger necessity evidence.</p>
