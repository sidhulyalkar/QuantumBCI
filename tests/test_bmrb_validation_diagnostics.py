...
def test_gate_diagnostics_localize_declared_failure_modes() -> None:
    ...
    assert cells["coverage-family-deficit"]["first_failing_gate_counts"]["coverage"] == 4
...
def test_gate_confusion_reports_supported_specificity_for_every_gate() -> None:
    result = run_bmrb_gate_diagnostics(diagnostic_policy())
    ...
    assert gates["coverage"]["expected_failure_support"] == 4
