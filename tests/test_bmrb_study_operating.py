...
def test_seed_authority_rejects_overlapping_stride_designs() -> None:
...
def test_declared_study_truths_cover_hierarchy_failure_modes() -> None:
    ...
    assert scenarios["redundant-positive-4"].expected_sensitivity_warning is False


def test_three_study_positive_is_successful_but_zero_margin_fragile() -> None:
    policy = _policy()
    scenario = next(
        item for item in default_study_operating_scenarios() if item.scenario_id == "homogeneous-positive-3"
    )
...
def test_fragile_conflict_and_redundant_positive_separate_sensitivity() -> None:
    policy = _policy()
    scenarios = {item.scenario_id: item for item in default_study_operating_scenarios()}
    ...
    redundant = run_study_operating_replicate(
        policy,
        scenarios["redundant-positive-4"],
        cell_index=3,
        replicate=0,
        participants=8,
        within_scale=0.0,
    ...
    assert redundant.replication_criteria_passed is True
    assert redundant.study_passes == (True, True, True, True)
    assert redundant.successful_replication_margin == 1
    assert redundant.sensitivity_warning is False
